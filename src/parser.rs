...
pub fn parse_bookmarks(bookmarks_file: &BookmarksFile) -> Vec<Bookmark> {
    ...
    flatten_node(&bookmarks_file.roots.bookmark_bar, "", &mut bookmarks);

    // Process other bookmarks
    flatten_node(&bookmarks_file.roots.other, "", &mut bookmarks);

    // Process synced bookmarks if present
    if let Some(synced) = &bookmarks_file.roots.synced {
        flatten_node(synced, "", &mut bookmarks);
    }
...
mod tests {
    ...
    fn test_normalize_url() {
        assert_eq!(
            normalize_url("[example.com](https://Example.COM/path/)"),
            "[example.com](https://example.com/path)"
        );
        assert_eq!(
            normalize_url("[test.com](http://test.com:80/page)"),
            "[test.com](http://test.com/page)"
        );
        assert_eq!(
            normalize_url("[test.com](https://test.com:443/)"),
    ...
    fn test_extract_domain() {
        assert_eq!(extract_domain("[example.com](https://www.example.com/path)"), "www.example.com");
        assert_eq!(extract_domain("[test.com](http://TEST.COM)"), "test.com");
